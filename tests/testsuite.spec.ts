...
test('Test case 03 - Delete Post by ID', async ({ request }) => {
  ...
  const allPosts = await getPostsResponse.json();
  //retrive the id of the last but one element in the array.
  ...
  const deletedElementRespons = await request.get(`http://localhost:3000/posts/${lastButOnePostID}`)
  expect (deletedElementRespons.status()).toBe(404);
  })
})